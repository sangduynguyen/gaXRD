...
# defines a typical gaussian function, of independent variable x=2theta, a=1,b=2theta_0, c=w
#Lorentzian as m → 1 and approaches a Gaussian as m → ∞
m=3
def lorentzV(x,a,b,c):
    #Lorentzian as m → 1 and approaches a Gaussian as m → ∞
    #m=4
    return a*c**(2*m)/((2**(1/m)-1)*(x-b)**2 + (c**2))**m
    # return a*c**2/((x-b)**2 + (c**2))

# defines the expected resultant as a sum of intrinsic gaussian functions
def GaussSum(x, p, n):
    return builtins.sum(lorentzV(x, p[3*k], p[3*k+1], p[3*k+2]) for k in range(n))
...
for i in range(n_value):
    ledger.append(
        'P' + str(i+1)
        + ', DA_WGL_PS' + str(i+1) +' = '+ str(round(AE2[i],2)) + ' nm'
...
print("DA_VGL_PS=",np.sum(DA1)/n_value," nm")
...
ax1.text(0.01, 0.99, r'Voigt Model (Lorentzian: m → 1, Gaussian: m → ∞ ) model'+', FOM = {0:0.6f}'
         .format(abs(FOM(cnsts, y, x, n_value)))+', m = {0:0.0f}'.format(m)+', DA_VGL_PS = {0:0.6f} nm\n'
         .format(np.sum(DA1)/n_value), transform=ax1.transAxes)
#ax1.text(0.01, 0.99, r'm = {0:0.0f}'
         #.format(m), transform=ax1.transAxes)
#ax1.text(0.80, 0.85, r'DA_PS = {0:0.6f} nm'
         #.format(np.sum(DA1)/n_value), transform=ax1.transAxes)
#ax1.text(0.80, 0.80, r'DA_IR = {0:0.6f} nm'
